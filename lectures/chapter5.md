...
신경망 학습(역전파)에서 그래디언트를 계산하려면 **각 레이어의 활성화 함수 도함수**가 필수입니다.

```
순전파: x → [선형] → z → [활성화] → a → [손실] → L

역전파: dL/dz = dL/da × da/dz
                      ↑
              활성화 함수의 도함수!
```

즉, 역전파에서 그래디언트가 활성화 함수를 통과할 때마다 **활성화 함수의 도함수**를 곱해야 합니다. 이 값이 너무 작으면 그래디언트 소실(vanishing gradient), 너무 크면 폭발(exploding gradient) 문제가 발생합니다.
